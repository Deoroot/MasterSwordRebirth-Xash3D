...
void ClientPutInServer(edict_t *pEntity)
{
...
#ifdef XASH_BUILD
	// Xash3D does not provide Steam auth IDs. The client detects its own SteamID64
	// via the Windows registry and sends it through the "ms_steamid" userinfo key.
	{
		const char* pszSteamID = g_engfuncs.pfnInfoKeyValue(
			g_engfuncs.pfnGetInfoKeyBuffer(pEntity), "ms_steamid");
		pPlayer->steamID64 = (pszSteamID && pszSteamID[0]) ?
			(ID64)strtoull(pszSteamID, nullptr, 10) : 0ULL;
	}
#else
	...
	char msg[256];
	_snprintf(msg, sizeof(msg), "Connecting %s (%s) [steamID64: %llu]\n", pPlayer->DisplayName(), pPlayer->m_ClientAddress, pPlayer->steamID64);
	g_engfuncs.pfnServerPrint(msg);
	logfile << Logger::LOG_INFO << msg;

	// Prevent multiple players from sharing steamID64=0, which would corrupt FN character data.
	if (pPlayer->steamID64 == 0)
	{
		logfile << Logger::LOG_INFO << "[ClientPutInServer] Kicking player: steamID64 is 0 (Steam not running or not logged in)\n";
		SERVER_COMMAND(UTIL_VarArgs("kick #%i \"Steam not detected. Please make sure Steam is running and you are logged in.\"\n",
			ENTINDEX(pEntity)));
		return;
	}
