...
#ifdef XASH_BUILD
	// Xash3D's R_DrawViewModel bails if viewent->model is NULL.
	// CL_LoadModel only searches the server model table; null.mdl may not be there.
	// Use Mod_ForName to load directly from disk, with player model as last resort.
	{
		if (!viewent->model)
		{
			viewent->model = IEngineStudio.Mod_ForName("models/null.mdl", 0);
			if (!viewent->model)
			{
				cl_entity_t* localPlayer = gEngfuncs.GetLocalPlayer();
				if (localPlayer && localPlayer->model)
					viewent->model = localPlayer->model;
			}
		}
	}
	// V_SetupViewModel sets colormap to player top/bottom colors which can cause
	// out-of-bounds access in Xash3D's remap color code. Clear it.
	viewent->curstate.colormap = 0;
#endif
