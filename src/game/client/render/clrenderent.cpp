...
#ifdef XASH_BUILD
// Gear items can't use CL_CreateVisibleEntity (entity number -1 is rejected).
// Instead they're queued here and rendered inline when their base entity is drawn.
#define XASH_GEAR_QUEUE_MAX 32
cl_entity_t* g_XashGearEnts[XASH_GEAR_QUEUE_MAX];
int          g_XashGearBaseNums[XASH_GEAR_QUEUE_MAX];
int          g_XashGearQueueCount = 0;
#endif
...
bool cl_entity_s::SetModel(const char *pszModelName)
{
	...
	// Use Mod_ForName to load models directly from disk as fallback.
	if (!model)
		model = IEngineStudio.Mod_ForName(pszModelName, 0);
#endif
...
#ifdef XASH_BUILD
	// CL_CreateVisibleEntity rejects number=-1 (unsigned overflow > MAX_EDICTS).
	// Queue for inline rendering inside the studio callback for the base entity.
	if (g_XashGearQueueCount < XASH_GEAR_QUEUE_MAX)
	{
		g_XashGearEnts[g_XashGearQueueCount] = &ItemEnt;
		g_XashGearBaseNums[g_XashGearQueueCount] = Ent.curstate.number;
...
void CRenderPlayerInset::Render()
{
	...
	*/
	...
	SetBits(m_Ent.curstate.MSR_CMAP, MSRDR_ASPLAYER | MSRDR_COPYPLAYER | MSRDR_GLOW_RED);
...
void CRenderPlayerInset::RenderGearItem(CGenericItem &Item)
{
	...
	SetBits(ItemEnt.curstate.MSR_CMAP, MSRDR_GLOW_RED);
	if (Item.m_Location == ITEMPOS_HANDS && player.m_CurrentHand == Item.m_Hand)
	{
		ClearBits(ItemEnt.curstate.MSR_CMAP, MSRDR_GLOW_RED);
		SetBits(ItemEnt.curstate.MSR_CMAP, MSRDR_GLOW_GRN);
	}
	else
	{
		ClearBits(ItemEnt.curstate.MSR_CMAP, MSRDR_GLOW_GRN);
	}
