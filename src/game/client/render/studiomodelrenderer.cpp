...
#ifdef XASH_BUILD
extern cl_entity_t* g_XashGearEnts[32];
extern int          g_XashGearBaseNums[32];
extern int          g_XashGearQueueCount;

// Renders and removes all queued gear items whose base entity matches base_num.
static void FlushGearQueue(int base_num)
{
	for (int i = 0; i < g_XashGearQueueCount; )
	{
...
void CStudioModelRenderer::StudioSetUpTransform(int trivial_accept)
{
	...
	// In Xash3D, entity baselines default to scale=1.0 (GoldSrc uses 0).
	// MSC server never sets scale for player entities, so Xash3D keeps scale=1.0
	// and interpolates toward the GoldSrc-sent value (0), producing oscillating
	// intermediate values (0.3, 0.7...) that incorrectly resize player models.
	// Skip scale entirely for player entities in Xash3D.
#ifdef XASH_BUILD
	if (Ent.curstate.scale && !Ent.player)
#else
...
int CStudioModelRenderer::StudioDrawModel(int flags)
{
	...
	if (FBitSet(m_pCurrentEntity->curstate.oldbuttons, MSRDR_VIEWMODEL))
	{
		...
		for (int hand = 0; hand < MAX_PLAYER_HANDITEMS; hand++) //Includes playerhands view model
		{
...
#ifdef XASH_BUILD
			// Xash3D's CL_LoadModel only searches the server model table.
			// Use Mod_ForName to load weapon viewmodels directly from disk.
			if (!pModel)
				pModel = IEngineStudio.Mod_ForName(pItem->m_ViewModel, 0);
#endif
			if (!pModel)
				continue;
...
#ifdef XASH_BUILD
			// Xash3D doesn't allow adding visible entities during the StudioDrawModel
			// rendering callback — CL_CreateVisibleEntity always returns 0 here.
			// MSR renders these entities itself in the loop below, so engine
			// registration is not needed. Just use the entity directly.
			// Do NOT memset — animation state (sequence, framerate, animtime) must
			// persist across frames since MSR only sets them on anim change.
#else
...
#ifdef XASH_BUILD
			// Xash3D crashes if viewent->model is NULL in R_DrawViewModel.
			// Set to null.mdl to suppress the engine's default viewmodel
			// while keeping it valid. MSR renders its own weapon viewmodels.
			view->model = IEngineStudio.Mod_ForName("models/null.mdl", 0);
#else
			view->model = NULL; //MiB JUN2010_21 - Disable the HL viewmodel. We use our own.
#endif

			// Set the specified submodel on the item.
			if (pStudioHeader && pItem->m_ViewModelPart < pStudioHeader->numbodyparts)
			{
				mstudiobodyparts_t* pbodypart = (mstudiobodyparts_t*)((byte*)pStudioHeader + pStudioHeader->bodypartindex) + pItem->m_ViewModelPart;
				if (pItem->m_ViewModelSubmodel < pbodypart->nummodels)
				{
					int iCurrent = (RenderEnt.curstate.body / pbodypart->base) % pbodypart->nummodels;
					RenderEnt.curstate.body = RenderEnt.curstate.body - (iCurrent * pbodypart->base) + (pItem->m_ViewModelSubmodel * pbodypart->base);
				}
			}
			...
			SetBits(RenderEnt.curstate.MSR_CMAP, MSRDR_HANDMODEL);
			SetBits(RenderEnt.curstate.oldbuttons, MSRDR_NOREFLECT);
#ifdef XASH_BUILD
			RenderEnt.curstate.colormap = 0;
#endif
	...
	for (int rm = 0; rm < TotalModels; rm++)
	{
		...
		if (FBitSet(Ent.curstate.MSR_CMAP, MSRDR_ASPLAYER))
		{
			...
			if (FBitSet(Ent.curstate.MSR_CMAP, MSRDR_COPYPLAYER))
			{
		...
		if (flags & STUDIO_RENDER)
		{
			...
			if (FBitSet(m_pCurrentEntity->curstate.MSR_CMAP, MSRDR_HANDMODEL))
			{
#ifdef XASH_BUILD
				// In Xash3D, IEngineStudio.StudioSetupModel reads curstate.body from the
				// engine's current entity (RI.currententity), which is the viewmodel entity
				// (null.mdl). MSR builds the correct body value in CLViewEntities[hand]
				// but that entity isn't registered with the engine. Sync body to the engine's
				// viewmodel entity so the correct weapon submodel gets rendered.
				cl_entity_s* engineView = gEngfuncs.GetViewModel();
				if (engineView)
					engineView->curstate.body = m_pCurrentEntity->curstate.body;
#endif
				float tmp[2];
...
#ifdef XASH_BUILD
			// Render any gear items queued for this entity (char selection, world models).
			FlushGearQueue(m_pCurrentEntity->curstate.number);
#endif
		}
	}
	enddbg;
...
void CStudioModelRenderer::StudioProcessGait(entity_state_t* pplayer)
{
	...
	if (!FBitSet(m_pCurrentEntity->curstate.MSR_CMAP, MSRDR_ASPLAYER))
	{
...
int CStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t* pplayer)
{
	...
	if (flags & STUDIO_RENDER)
	{
		...
		if( pplayer->number == gEngfuncs.GetLocalPlayer()->index &&
			!FBitSet(m_pCurrentEntity->curstate.MSR_CMAP,MSRDR_ASPLAYER) )
		{
...
#ifdef XASH_BUILD
		// Render any gear items queued for this player entity.
		FlushGearQueue(m_pCurrentEntity->curstate.number);
#endif
...
void CStudioModelRenderer::StudioRenderModel(void)
{
	...
	if (FBitSet(m_pCurrentEntity->curstate.MSR_CMAP, MSRDR_GLOW_GRN))
		lighting.color = Vector(0, 1, 0);
	if (FBitSet(m_pCurrentEntity->curstate.MSR_CMAP, MSRDR_GLOW_RED))
		lighting.color = Vector(1, 0, 0);
	if (FBitSet(m_pCurrentEntity->curstate.MSR_CMAP, MSRDR_DARK))
		lighting.ambientlight = 0;

	if (FBitSet(m_pCurrentEntity->curstate.MSR_CMAP, MSRDR_LIGHT_NORMAL))
	{
	...
	if (FBitSet(m_pCurrentEntity->curstate.MSR_CMAP, MSRDR_LIGHT_DIM))
	{
	...
	// get remap colors
#ifdef XASH_BUILD
	// Xash3D: Skip remap entirely. MSR doesn't use HL player color remapping,
	// and CL_EntitySetRemapColors crashes because client-created entities have
	// curstate.number values that are out of bounds for clgame.remap_info[].
	m_nTopColor = 0;
	m_nBottomColor = 0;
#else
