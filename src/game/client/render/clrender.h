...
//Separate flags from the above -- stored in curstate.colormap (or curstate.iuser4 for Xash3D)
...
// Xash3D's engine reads curstate.colormap for player model color remapping (R_StudioSetRemapColors).
// MSR repurposes colormap for rendering flags, which causes crashes in CL_EntitySetRemapColors.
// Under Xash3D, store MSR flags in iuser4 instead to keep colormap clean for the engine.
#ifdef XASH_BUILD
#define MSR_CMAP iuser4
#else
#define MSR_CMAP colormap
#endif
