...
void DLLEXPORT HUD_Init(void)
{
...
#ifdef XASH_BUILD
	// Detect SteamID64 from the Windows registry (requires Steam to be running and logged in).
	// ActiveProcess\ActiveUser is a DWORD = SteamID3 account ID (= SteamID64 - 76561197960265728).
	// The engine syncs FCVAR_USERINFO cvars to the server automatically on connect.
	unsigned long long detectedSteamID64 = 0ULL;
	HKEY hKey;
	                  ...
	                  0, KEY_READ, &hKey) == ERROR_SUCCESS)
	{
		...
		RegQueryValueExA(hKey, "ActiveUser", NULL, NULL, (LPBYTE)&activeUser, &size);
		RegCloseKey(hKey);
		if (activeUser != 0)
			detectedSteamID64 = (unsigned long long)activeUser + 76561197960265728ULL;
	}
	char szSteamID64[32];
	_snprintf(szSteamID64, sizeof(szSteamID64), "%llu", detectedSteamID64);
	// Write directly into the local player's userinfo buffer.
	// This bypasses the cvar system entirely, so config.cfg can never override it.
	gEngfuncs.PlayerInfo_SetValueForKey("ms_steamid", szSteamID64);
	logfile << Logger::LOG_INFO << "[HUD_Init: ms_steamid=" << szSteamID64 << "]\n";
#endif
