...
#ifdef XASH_BUILD
	// Xash3D FWGS: SPR_Draw* needs absolute screen coords since vgui_support
	// doesn't set up GL translation for the VGUI paint context.
	int drawX = coords[0] + Ofs[0];
	int drawY = coords[1] + Ofs[1];
#else
	int drawX = Ofs[0];
	int drawY = Ofs[1];
#endif
	if( fgColor[3] >= 255 )
		SPR_DrawHoles( m_Frame, drawX, drawY, &DisplaySize );
	else
		SPR_DrawAdditive( m_Frame, drawX, drawY, &DisplaySize );	
}
