...
void CRenderChar::Init( int Idx, msstring model )
{
	...
	SetBits( m_Ent.curstate.oldbuttons, MSRDR_NOREFLECT );
 	SetBits( m_Ent.curstate.MSR_CMAP, MSRDR_ASPLAYER );
...
void CRenderChar::Render( )
{
	...
	// Body is set below, after gear is resolved and before rendering.
	// (Previously it was reset to 0 here and set again after render, so render always saw body=0.)
	...
	if( m_Stage != STG_CHOOSEGENDER )
	{
		...
		for (int i = 0; i < player.m_CharInfo[m_Idx].GearInfo.size(); i++) 
		{
			...
			ItemEnt.curstate.MSR_CMAP = ItemEnt.curstate.oldbuttons = 0;
			SetBits( ItemEnt.curstate.oldbuttons, MSRDR_NOREFLECT );
			SetBits( ItemEnt.curstate.MSR_CMAP, MSRDR_LIGHT_NORMAL );
	...
	// Body must be set BEFORE rendering so the correct submodel is visible.
	// For existing characters, use the saved gender; for gender selection, use m_Gender.
	{
		gender_e gender = (m_Stage != STG_CHOOSEGENDER)
			? player.m_CharInfo[m_Idx].Gender
			: m_Gender;
		int gv = (gender == GENDER_MALE) ? 1 : 2;
		m_Ent.SetBody(0, gv);
		m_Ent.SetBody(1, gv);
		m_Ent.SetBody(2, gv);
		m_Ent.SetBody(3, gv);
...
{
	m_Active = Active;
	ClearBits( m_Ent.curstate.MSR_CMAP, MSRDR_LIGHT_NORMAL|MSRDR_LIGHT_DIM );

	if( Active )
	{
		PlayAttnAnim( );
		Highlight( false );
		SetBits( m_Ent.curstate.MSR_CMAP, MSRDR_LIGHT_NORMAL );
	}
	else
	{
		...
		SetBits( m_Ent.curstate.MSR_CMAP, MSRDR_LIGHT_DIM );
	}
}
